...
매일 아침 예약 실행되는 Claude 세션이 이 문서대로 작업한다.
...
- `examples/01-미루는이유.json` 완성된 1편. 데이터 형식과 문장 길이의 기준.
...
다음 번호 = log.md 마지막 번호 + 1 (두 자리, 예: 02).
...
- `sheet.cells`: `p` 2줄 줄당 9자 이내, `a` 9자 이내.
...
### 7. 기록
`log.md` 에 한 줄 추가 (번호, 날짜, 주제, 확인 페이지 링크).
