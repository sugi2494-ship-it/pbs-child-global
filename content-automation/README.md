...
- 표지 `title`: **그 편의 상황을 말하는 제목** (예: `명절 끝나고<br><span class="m">유독 지친</span> 이유`, `답장이 늦는<br><span class="m">진짜 이유</span>`). 2줄, 줄당 7자 이내, 핵심 말만 마젠타. "당신은 몇 번이에요?"처럼 매번 같은 제목은 쓰지 않는다 (피드에서 반복돼 보임, 대표 결정). "나는 몇 번일까요?"는 템플릿이 목록 위에 자동으로 넣는다.
